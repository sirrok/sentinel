...
def has_zalias_conf():
    ...
    valid_zalias_conf = False

    # ensure zalias_conf exists & readable
    #
    # if not, print a message stating that Zalias Core must be installed and
    # configured, including JSONRPC access in zalias.conf
    try:
        f = io.open(config.zalias_conf)
        valid_zalias_conf = True
    except IOError as e:
        print(e)

    return valid_zalias_conf
...
def main():
    ...
    if not has_zalias_conf():
        print("Zalias must be installed and configured, including JSONRPC access in zalias.conf")
        sys.exit(1)
