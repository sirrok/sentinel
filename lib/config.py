...
sentinel_cfg = ZaliasConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_zaliasd_proto_version_with_sentinel_ping = 80000


def get_zalias_conf():
    home = os.environ.get('HOME')

    zalias_conf = os.path.join(home, ".zaliascore/zalias.conf")
    if sys.platform == 'darwin':
        zalias_conf = os.path.join(home, "Library/Application Support/Zalias/zalias.conf")

    zalias_conf = sentinel_cfg.get('zalias_conf', zalias_conf)
