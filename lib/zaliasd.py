...
class ZaliasDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some zaliasd methods
    ...
    @classmethod
    def from_zalias_conf(self, zalias_dot_conf):
        from zalias_config import ZaliasConfig
        config_text = ZaliasConfig.slurp_config_file(zalias_dot_conf)
        creds = ZaliasConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = zaliaslib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = zaliaslib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = zaliaslib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return False # (getinfo['protocolversion'] >= config.min_zaliasd_proto_version_with_sentinel_ping)
