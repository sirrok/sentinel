...
def is_valid_zalias_address(address, network='mainnet'):
    ...
    zalias_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Zalias address
        return False

    if (address_version != zalias_version):
        return False
...
# shims 'til we can fix the zaliasd side
def SHIM_serialise_for_zaliasd(sentinel_hex):
    ...
    # superblock => "trigger" in zaliasd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # zaliasd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    zaliasd_hex = serialise(obj)
    return zaliasd_hex


# shims 'til we can fix the zaliasd side
def SHIM_deserialise_from_zaliasd(zaliasd_hex):
    from models import COIN2FLYD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(zaliasd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('zalias.conf')
