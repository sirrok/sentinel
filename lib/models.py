...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, zaliasd):
        golist = zaliasd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_zaliasd(zaliasd, item)
    ...
    @classmethod
    def import_gobject_from_zaliasd(self, zaliasd, rec):
        import decimal
        import zaliaslib
        import inflection
        ...
        # shim/zaliasd conversion
        object_hex = zaliaslib.SHIM_deserialise_from_zaliasd(object_hex)
        objects = zaliaslib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from zaliasd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from zaliasd, with every run
        ...
        # get/create, then sync payment amounts, etc. from zaliasd - Zaliasd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(zaliasd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from zaliasd! %s" % e)
            govobj.vote_delete(zaliasd)
            return (govobj, None)
    ...
    def vote_delete(self, zaliasd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(zaliasd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, zaliasd, signal, outcome):
        import zaliaslib
        ...
        output = zaliasd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = zaliaslib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(zaliasd, signal)

    def sync_network_vote(self, zaliasd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = zaliasd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 zalias addr, non-multisig
            if not zaliaslib.is_valid_zalias_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Zalias address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (zaliaslib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/ZaliasDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import zaliaslib
        obj_data = zaliaslib.SHIM_serialise_for_zaliasd(self.serialise())
    ...
    def prepare(self, zaliasd):
        try:
            object_hash = zaliasd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not zaliaslib.is_valid_zalias_address(addr, config.network):
    ...
    def hash(self):
        import zaliaslib
        return zaliaslib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, zaliasd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - zaliasd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, zaliasd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - zaliasd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, zaliasd):
        now = int(time.time())
        return (self.created_at < (now - zaliasd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, zaliasd):
        if self.is_expired(zaliasd):
            return False

        return True

    def is_deletable(self, zaliasd):
        if self.is_expired(zaliasd):
            return True
